...
void main() {
  test('proof visual themes are unique data bundles without geometry', () {
    ...
    for (final theme in themes) {
      ...
      expect(theme.backgroundScale, greaterThanOrEqualTo(1));
    }
  ...
  test('Orman production skin is separate from proof presets', () {
    expect(WordHuntRouteVisualThemes.ormanYolu.id, 'orman-yolu-production');
    expect(
      WordHuntRouteVisualThemeProofs.all
          .map((theme) => theme.id)
          .contains(WordHuntRouteVisualThemes.ormanYolu.id),
      isFalse,
    );
  });
  ...
  testWidgets(
    'raster artwork stays clean while live nodes and locked state remain dynamic',
    (tester) async {
      tester.view.physicalSize = const Size(390, 844);
      tester.view.devicePixelRatio = 1;
      addTearDown(() {
      ...
      expect(
        find.byKey(const Key('word_hunt_route_background_scale')),
        findsOneWidget,
      );
      expect(
      ...
      expect(
        ...
        findsNothing,
        reason:
            'Final artwork üzerinde tekrar procedural ağaç/mantar çizilmemeli.',
      );
      expect(
  ...
  testWidgets('real Orman Yolu content renders with production skin', (
    tester,
  ) async {
    tester.view.physicalSize = const Size(390, 844);
    tester.view.devicePixelRatio = 1;
