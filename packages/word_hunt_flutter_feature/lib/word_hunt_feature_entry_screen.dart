...
class WordHuntFeatureEntryScreen extends StatefulWidget {
  const WordHuntFeatureEntryScreen({
    super.key,
    this.ownerUid,
  ...
  State<WordHuntFeatureEntryScreen> createState() =>
      _WordHuntFeatureEntryScreenState();
}

class _WordHuntFeatureEntryScreenState
    extends State<WordHuntFeatureEntryScreen> {
  ...
  Future<void> _loadProgress() async {
    ...
    try {
      final raw = await widget.progressStore.getString(_storageKey);
      ...
      if (_catalogMode && hadPersistedProgress) {
        ...
        final revealSeen =
            await widget.progressStore.getBool(_kristalRevealSeenKey) ?? false;
        shouldRevealHistoricalKristal =
            kristal.isUnlocked(loaded) && !kristalHasProgress && !revealSeen;
        if (shouldRevealHistoricalKristal) {
          await widget.progressStore.setBool(_kristalRevealSeenKey, true);
        }
      }
  ...
  Future<void> _saveProgress(WordHuntProgressSnapshot progress) async {
    try {
      await widget.progressStore.setString(
        _storageKey,
