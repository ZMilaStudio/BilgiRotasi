...
class WordHuntRouteDecorationPainter extends CustomPainter {
  ...
  void _paintCliffsAndWaterfalls(Canvas canvas, Size size) {
    ...
    final mossPaint =
        Paint()..color = palette.primary.withValues(alpha: opacity * 0.64);
  ...
  void _paintBridge(Canvas canvas, Size size) {
    ...
    final shadowPaint =
        Paint()..color = Colors.black.withValues(alpha: opacity * 0.34);
    canvas.drawOval(
      Rect.fromCenter(
    ...
    final plankPaint =
        Paint()..color = palette.secondary.withValues(alpha: opacity * 0.92);
    final plankLightPaint =
        Paint()
    ...
    for (final dx in <double>[-0.5, -0.25, 0.0, 0.25, 0.5]) {
      ...
      canvas.drawLine(Offset(x, center.dy - 3), Offset(x, railY), ropePaint);
    }
  }

  void _paintNodePedestals(Canvas canvas, Size size) {
    final shadowPaint =
        Paint()..color = Colors.black.withValues(alpha: opacity * 0.42);
    final barkPaint =
        Paint()..color = palette.secondary.withValues(alpha: opacity * 0.88);
    final woodPaint =
        Paint()
  ...
  void _paintEdgeTrees(Canvas canvas, Size size) {
    ...
    final leafMid =
        Paint()..color = palette.primary.withValues(alpha: opacity * 0.84);
    final leafLight =
        Paint()
    ...
    for (var i = 0; i < 34; i++) {
      ...
      final paint =
          i % 3 == 0
              ...
              : leafDark;
      canvas.drawOval(
        Rect.fromCenter(center: Offset(x, y), width: r * 1.35, height: r),
        paint,
      );
    }
  }
  ...
  void _paintHarborScene(Canvas canvas, Size size) {
    ...
    for (var band = 0; band < 8; band++) {
      ...
      for (var step = 0; step < 5; step++) {
        ...
        wave.quadraticBezierTo((startX + endX) / 2, controlY, endX, y);
      }
  ...
  void _paintMushrooms(Canvas canvas, double scale) {
    ...
    for (final x in <double>[-7, 6]) {
      ...
      canvas.drawRRect(
        RRect.fromRectAndRadius(
          Rect.fromLTWH((x - 2) * scale, -1 * scale, 4 * scale, h * scale),
      ...
      canvas.drawCircle(Offset((x - 2) * scale, -2 * scale), 1.2 * scale, spot);
      canvas.drawCircle(Offset((x + 3) * scale, 0), 1.0 * scale, spot);
    }
  }
  ...
  void _paintFlowers(Canvas canvas, double scale) {
    ...
    final center =
        Paint()..color = palette.accent.withValues(alpha: opacity * 0.92);
    for (var i = -2; i <= 2; i++) {
      final x = i * 5.0 * scale;
  ...
  void _paintShrub(Canvas canvas, double scale) {
    ...
    final light =
        Paint()..color = palette.primary.withValues(alpha: opacity * 0.86);
    for (var i = 0; i < 7; i++) {
  ...
  void _paintSkyMotif(Canvas canvas, double scale, int variantIndex) {
    if (variantIndex.isEven) {
      final cloud =
          Paint()..color = palette.primary.withValues(alpha: opacity * 0.48);
      ...
      final star =
          Paint()..color = palette.accent.withValues(alpha: opacity * 0.76);
  ...
  void _paintHarborMotif(Canvas canvas, double scale, int variantIndex) {
    if (variantIndex.isEven) {
      final post =
          Paint()..color = palette.secondary.withValues(alpha: opacity * 0.82);
      canvas.drawRRect(
        RRect.fromRectAndRadius(
          Rect.fromCenter(
            center: Offset.zero,
      ...
      final lamp =
          Paint()..color = palette.accent.withValues(alpha: opacity * 0.80);
